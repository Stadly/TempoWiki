...
Congratulations! You have now successfully installed and tried out the most important features of TempoWiki.
...
In the newest versions of Spotify, support for Apps has been removed. In order to use TempoWiki, you must therefore use an older version of Spotify.
How to install an older version of Spotify and prevent it from automatically updating:
1. Install Spotify from the file 'SpotifyFullSetup.exe'. This will install Spotify 0.9.15.27.g87efe634
2. Navigate to '%APPDATA%/Spotify'
3. Create an empty text file and name it 'Spotify_new.exe'
5. Make 'Spotify_new.exe' read-only

How to create a shortcut that will automatically load TempoWiki when starting Spotify:
1. Create a shortcut pointing to '%APPDATA%/Spotify/spotify.exe'
2. Append '-uri "spotify:app:tw"' to the Target for the shortcut
